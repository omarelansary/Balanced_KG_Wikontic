...
def get_args():
    ...
    parser.add_argument("--dataset_path", type=str, default="datasets/musique_200_test.json")
    ...
    parser.add_argument("--no_use_qualifiers", action="store_false", dest="use_qualifiers", help="Do not use qualifiers")
    parser.add_argument("--use_filtered_triplets", action="store_true", help="Use filtered triplets")
    parser.add_argument("--no_use_filtered_triplets", action="store_false", dest="use_filtered_triplets", help="Do not use filtered triplets")
...
if __name__ == "__main__":
    ...
    if args.multi_step_qa:
        ...
        for sample_id in tqdm(unique_sample_ids):
            try:
                ...
                with jsonlines.open(f"qa_logs/{args.triplets_db_name}_{args.model_name.replace('/', '_')}_structured_{args.structured_inference}_multi_step_{args.multi_step_qa}_use_qualifiers_{args.use_qualifiers}_use_filtered_triplets_{args.use_filtered_triplets}_musique_test_run_{str(args.run_number)}.jsonl", "a") as f:
    ...
    # with open(f"qa_results/{args.triplets_db_name}_{args.model_name.replace('/', '_')}_structured_{args.structured_inference}_multi_step_{args.multi_step_qa}_use_qualifiers_{args.use_qualifiers}_use_filtered_triplets_{args.use_filtered_triplets}_musique_200_test_run_{str(args.run_number)}.json", "w") as f:
