...
def get_args():
    ...
    parser.add_argument("--intial_triplets_db_name", type=str, default="intial_triplets_db")
...
if __name__ == "__main__":
    ...
    if args.intial_triplets_db_name:
        initial_triplets_db = mongo_client.get_database(args.intial_triplets_db_name)
        logger.info(f"Initial triplets db: {args.intial_triplets_db_name}")
    else:
        initial_triplets_db = None
        logger.info("Initial triplets db: None")
    ...
    if args.structured_inference:
        ...
        inference_with_db = InferenceWithDB(extractor=extractor, aligner=aligner, triplets_db=triplets_db, initial_triplets_db=initial_triplets_db)
    ...
    sampled_ids = list(id2sample.keys())[138:num_samples]
